...
typedef double FloatLatitude;
typedef double FloatLongitude;
...
inline FloatLatitude clampLat(const FloatLatitude lat)
{
    return std::max(std::min(lat, FloatLatitude{detail::MAX_LATITUDE}),
                    FloatLatitude{-detail::MAX_LATITUDE});
}

inline FloatLongitude clampLon(const FloatLongitude lon)
{
...
inline double lonToPixel(FloatLongitude lon, unsigned zoom)
{
...
inline double latToPixel(FloatLatitude lat, unsigned zoom)
{
    const double shift = (1u << zoom) * TILE_SIZE;
    const double b = shift / 2.0;
    ...
    const int x, const int y, const int z, double &minx, double &miny, double &maxx, double &maxy)
{
    ...
    minx = static_cast<double>(clampLon(FloatLongitude{minx})) * DEGREE_TO_PX;
    miny = latToY(clampLat(FloatLatitude{miny})) * DEGREE_TO_PX;
    maxx = static_cast<double>(clampLon(FloatLongitude{maxx})) * DEGREE_TO_PX;
    maxy = latToY(clampLat(FloatLatitude{maxy})) * DEGREE_TO_PX;
}
}
}
